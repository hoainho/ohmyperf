...
__export(extension_exports, {
  ...
  measureUrlCommand: () => measureUrlCommand,
  openReportCommand: () => openReportCommand
});
...
function getConfig() {
  ...
  return {
    cliPath: cfg.get("cliPath", "").trim(),
    defaultUrl: cfg.get("defaultUrl", "http://localhost:3000"),
    defaultRuns: cfg.get("defaultRuns", 3),
    defaultMode: cfg.get("defaultMode", "real")
  };
}
...
function runCli(args) {
  return new Promise((resolve, reject) => {
    const child = (0, import_node_child_process.spawn)(resolveCliBinary(), [...args], {
...
async function loadReportFromFile(path) {
...
function showReportInWebview(context, report) {
  const panel = vscode.window.createWebviewPanel("ohmyperfReport", `OhMyPerf \u2014 ${report.meta.url}`, vscode.ViewColumn.Active, {
    enableScripts: false,
...
async function measureUrlCommand(context) {
  const cfg = getConfig();
  ...
  if (!url)
    return;
  const outDir = (0, import_node_path.join)((0, import_node_os.tmpdir)(), `ohmyperf-vscode-${String(process.pid)}-${String(Date.now())}`);
  ...
  try {
    ...
    if (cfg.defaultRuns === 1)
      args.push("--allow-single-run");
    let res;
    ...
    if (res.exitCode !== 0) {
      ...
      vscode.window.showErrorMessage(`OhMyPerf failed (exit ${String(res.exitCode)}): ${lines || "no stderr output"}`);
      return;
    }
    const report = await loadReportFromFile((0, import_node_path.join)(outDir, "report.json"));
    showReportInWebview(context, report);
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    if (message.includes("ENOENT") || message.includes("not found")) {
      const action = await vscode.window.showErrorMessage("OhMyPerf CLI not found on PATH. Install it or set 'ohmyperf.cliPath' in settings.", "Open settings");
    ...
    await (0, import_promises.rm)(outDir, { recursive: true, force: true }).catch(() => void 0);
  }
}
async function openReportCommand(context) {
  ...
  try {
    ...
    vscode.window.showErrorMessage(`Could not open report: ${err instanceof Error ? err.message : String(err)}`);
  }
}
function activate(context) {
  context.subscriptions.push(vscode.commands.registerCommand("ohmyperf.measureUrl", () => measureUrlCommand(context)), vscode.commands.registerCommand("ohmyperf.openReport", () => openReportCommand(context)));
}
function deactivate() {
  return void 0;
}
