...
- **`packages/sourcemaps/`** — NEW workspace package, name `@nhonh/sourcemaps`. Pulls in `@jridgewell/trace-mapping` (~20KB, zero deps; chosen over Mozilla `source-map` which is WASM-async, and over `@jridgewell/source-map` which is just a wrapper). Exposes:
...
- **`@nhonh/sourcemaps` is a new workspace package, not part of `@nhonh/core`**. Reason: pulls `node:fs`/`node:crypto` and we must keep `core` browser-safe for the extension + viewer bundles.
...
- **Diagnoser is in `@nhonh/plugins-builtin`, not core.** The taxonomy registry lives in core (leaf, browser-safe), but the *application* of the registry (mapping audits → diagnoses + running absence checks) belongs in the plugin layer. This keeps `core` engine-only and keeps the LLM-native surface in user-extensible territory.
- **Path-traversal hardening**: `@nhonh/sourcemaps` refuses any resolved `.map` path outside `config.repoRoot`. Absolute paths outside root → reject. `../` escapes → reject. Symlinks resolved before the boundary check.
