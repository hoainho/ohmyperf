...
- [ ] 2.7 Build the OOPIF synthetic test corpus under `tests/oopif-corpus/fixtures/`: parent + 3 cross-origin OOPIFs, sandboxed-no-scripts, srcdoc, fenced-frame, BFCache, prerender, SW-precache, SPA soft-nav, popup, worker, iframe-removed-mid-run, iframe-resize-causes-parent-shift. **(PARTIAL → Track A — A5)** — 4/13 fixtures present (`oopif-3-cross-origin`, `sandbox-no-scripts`, `srcdoc-iframe`, `iframe-removed-mid-run`); fixtures served procedurally via `fixture-server.ts`. The other 9 fixtures + Chromium feature-flag setup are owned by Track A tasks A5.1–A5.12.
...
- [ ] 3.2 Implement CWV collector via `web-vitals/attribution`. **(PARTIAL → Track A — A1)** — current [`cwv-collector.ts`](../../../packages/core/src/collectors-impl/cwv-collector.ts) uses custom PerformanceObservers (and **wrong INP algorithm**). Track A A1.1–A1.5 replaces with the official `web-vitals/attribution` library.
...
- [ ] 3.6 Implement runtime-breakdown (jsExec/layout/paint/composite/idle) from trace events. **(→ Track B — B1)** — requires `Tracing.start` + MainThreadTasks parser. Owned by Track B B1 (trace collector + tracehouse vendoring).
- [ ] 3.7 Implement memory collector via `Performance.getMetrics` and `Memory.getDOMCounters`. **(→ Track A — A3)** — `Performance.getMetrics` call already exists at `loading-collector.ts:92` but result is **discarded**. Track A A3.1–A3.3 captures the result. `Memory.getDOMCounters` → **v1.1 deferred** (orthogonal correctness surface, not blocking trustable CWV).
...
- [ ] 3.11 Implement frame-level CLS attribution to iframe-resize. **(→ Track A — A2.5)** — owned by Track A A2.5 (via `LayoutShift.sources[].node`, NOT the phantom `Page.frameResized` event).
...
- [ ] 6.11 Implement trace cap (warn 50MB, refuse >500MB) and heap cap. **(→ Track B — B1.4)** — Track B B1.4 implements 25MB warn / 100MB hard refuse cap (stricter than this task's 50/500MB; aligned with realistic IndexedDB quota).

## 7. P0 — Vendored trace utils + viewer skeleton

- [ ] 7.1 Vendor tracium-equivalent into `packages/trace-utils/`. **(→ Track B — B1.1)** — current stub is 2 LOC. Track B B1.1 vendors Lighthouse 13 `tracehouse` (≈500 LOC).
...
- [ ] 13.3 Reproducibility acceptance: 10× CI Stable runs, CoV ≤ 0.05 LCP / ≤ 0.10 INP. **(→ Track A — A4)** — owned by Track A parity test suite; current `calibration.test.ts` covers algorithm but not full 10-run real-page acceptance.
- [ ] 13.4 CWV-vs-Lighthouse parity acceptance ±10% LCP/FCP/TTFB. **(→ Track A — A4.3)** — owned by Track A.
