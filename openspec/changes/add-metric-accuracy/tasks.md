...
- [x] A0.4 web-vitals dependency ownership — **architecturally relocated**: instead of staying in `plugins-builtin` as the spec proposed, `web-vitals` moved to `@nhonh/core` `dependencies` (since `CWV_INLINE_SCRIPT` lives in core, not plugins-builtin). `packages/plugins-builtin` no longer needs web-vitals as a direct dependency — the cwv plugin delegates to core's collector. Verified clean install + api:check pass.
...
- [x] A5.12 Acceptance: `pnpm test --filter @nhonh/oopif-corpus` green with 13 fixtures.

## A6. API freeze + docs

- [x] A6.1 Run `pnpm api:check` on `@nhonh/core`. The `MetricAttribution` extension is additive; api-extractor should pass without breakage.
...
- [x] A7.5 `pnpm typecheck && pnpm lint && pnpm test && pnpm test:parity` — **PARTIAL**: `pnpm typecheck` 39/39 packages green ✓, `pnpm test` on `@nhonh/core` 39/39 tests green ✓, `pnpm lint` clean on changed packages ✓. `pnpm test:parity` requires Chromium (deferred to local). `pnpm test --filter @nhonh/oopif-corpus` requires Chromium (deferred to local).
